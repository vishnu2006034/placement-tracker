...
export const DsaTrackerView: React.FC<DsaTrackerViewProps> = ({
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 bg-slate-950 px-3.5 py-1.5 rounded-xl border border-slate-800">
            <span className="text-xs font-semibold text-slate-300">
              Total Solved: <strong className="text-emerald-400 font-extrabold">{totalSolved}</strong> / {dsaGoal}
            </span>
          ...
          return (
            ...
            >
              <div>
                ...
                {/* Progress bar */}
                <div className="w-full bg-slate-950 rounded-full h-1.5 overflow-hidden mt-2.5">
                  <div
                    className="h-full bg-emerald-400 rounded-full transition-all duration-300"
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {topic.keyConcepts.map((concept, idx) => (
                    <span
                      key={idx}
                      className="text-[10px] font-medium px-2 py-0.5 rounded-md bg-slate-950 text-slate-300 border border-slate-800/80"
                    >
                      {concept}
                    </span>
                  ))}
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800/80">
                ...
                <div className="flex items-center gap-1.5 bg-slate-950 p-1 rounded-lg border border-slate-800">
                  <button
                    ...
                    className="w-6 h-6 flex items-center justify-center rounded bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors cursor-pointer"
                  ...
                  <button
                    onClick={() => onUpdateTopicSolved(topic.id, 1)}
                    className="w-6 h-6 flex items-center justify-center rounded bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold transition-colors cursor-pointer"
      ...
      {/* Revision Pro Tip */}
      <div className="flex items-start gap-3 p-4 rounded-2xl bg-slate-900 border border-slate-800 text-xs text-slate-300">
        <Lightbulb className="w-4 h-4 text-amber-400 flex-shrink-0 mt-0.5" />
        <div>
          <strong className="text-white block mb-0.5">Campus Assessment Strategy:</strong>
          Aim for 120-180 problems across high-frequency patterns (Two Pointers, Sliding Window, Monotonic Stack, BFS/DFS, Tree LCA, and 0/1 Knapsack). Quality and pattern recognition beats brute-forcing 500+ random problems.
        </div>
      </div>
    </div>
  );
